...
{
    add_menu_page('Affiliate Link Cloaking','Link Cloaking','manage_options', __FILE__,'menu_main', get_bloginfo('url').'/wp-content/plugins/affiliate-link-cloaking/img/AffIcon_16.png' );
    add_submenu_page(__FILE__,'Add New Link','Add New Link','manage_options','affiliate-link-cloaking/ui_addnewlink.php','submenu_addnewlink');
    add_submenu_page(__FILE__,'Link Status','Link Status','manage_options','affiliate-link-cloaking/ui_track.php','submenu_linktrack');
    add_submenu_page(__FILE__,'Options','Options','manage_options','affiliate-link-cloaking/ui_options.php','submenu_options');
}
...
function afflc_autoshortlink($content)
{
    ...
    foreach( $alllinks as $onelink )
    {
        if ( 1 == $onelink->autoshortlink )
        {
           ...
           $shortlink=get_bloginfo('url').'/'.$onelink->cloaklink;
