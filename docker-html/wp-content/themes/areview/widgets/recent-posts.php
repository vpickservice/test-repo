...
// constructor
	...
	<p><label for="<?php echo $this->get_field_id( 'number' ); ?>"><?php _e( 'Number of posts to show:', 'areview' ); ?></label>
	<input id="<?php echo $this->get_field_id( 'number' ); ?>" name="<?php echo $this->get_field_name( 'number' ); ?>" type="text" value="<?php echo $number; ?>" size="3" /></p>

	<p><input class="checkbox" type="checkbox" <?php checked( $show_date ); ?> id="<?php echo $this->get_field_id( 'show_date' ); ?>" name="<?php echo $this->get_field_name( 'show_date' ); ?>" />
	<label for="<?php echo $this->get_field_id( 'show_date' ); ?>"><?php _e( 'Display post date?', 'areview' ); ?></label></p>
	...
	function update($new_instance, $old_instance) {
		...
		if ( isset($alloptions['areview_Recent_Posts']) )
			delete_option('areview_Recent_Posts');		  
		  
		return $instance;
	}
	
	function flush_widget_cache() {
		wp_cache_delete('areview_Recent_Posts', 'widget');
	}
	...
	function widget($args, $instance) {
		$cache = array();
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'areview_Recent_Posts', 'widget' );
		}
		...
		$title = ( ! empty( $instance['title'] ) ) ? $instance['title'] : __( 'Recent Posts', 'areview' );
...
?>
	...
	<?php
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'areview_Recent_Posts', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	}
