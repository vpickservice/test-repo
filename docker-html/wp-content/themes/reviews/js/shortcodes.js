(function($) {
	...
	$(document).on( 'click', '.shortcode-save-options', function(e){
		...
		$dialog.find('.shortcode-field').each(function(){
			var $this = $(this);
			if( ( $this.attr( 'name' ) == 'toggle_content' || $this.attr( 'name' ) == 'contents' ) && $this.val() ){
				content = $this.val();
			}
			else{
				params.push( $this.attr('name')+'="'+( $this.val() == null ? '' : $this.val() )+'"' );
			}
		});
	...
	if( typeof tinymce != 'undefined' ){
	    tinymce.create('tinymce.plugins.reviews', {
	        init : function(ed, url) {
				...
				/* elements */
				ed.addButton('reviewselements', {
					type: 'listbox',
					text: 'Reviews Elements',
					icon: false,
					onselect: function(e) {
						active_shortcode = this.value();
						call_shortcode();
					...
					values: [
						...
						{ text: 'Categories List', value: 'categories_list' },
						{ text: 'Gap', value: 'gap' },
						{ text: 'Gallery', value: 'bg_gallery' },
						{ text: 'Icon', value: 'icon' },
						{ text: 'Iframe', value: 'iframe' },
						{ text: 'Label', value: 'label' },
						{ text: 'Latest Blogs', value: 'latest_blogs' },
				...
				if( pagenow == 'mega_menu' ){
					ed.addButton('reviewselementsmega', {
						type: 'listbox',
						text: 'Reviews Mega Menu Elements',
						icon: false,
						onselect: function(e) {
							active_shortcode = this.value();
							call_shortcode();
						...
						values: [
							{ text: 'Mega Menu Reviews', value: 'mega_menu_reviews' },
						],
						onPostRender: function() {
							ed.my_control = this;
						}
